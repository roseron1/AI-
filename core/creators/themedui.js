...
( function() {
	...
	function createInstance( element, config, data, mode ) {
		...
		editor.on( 'loaded', function() {
            window.hmEditor && window.hmEditor.registerToolbar();
			loadTheme( editor );
	...
	function loadTheme( editor ) {
		...
		var container = CKEDITOR.dom.element.createFromHtml( themedTpl.output( {
			...
			topHtml: topHtml ? '<span id="' + editor.ui.spaceId( 'top' ) + '" class="cke_top cke_reset_all" role="presentation" style="height: ' + (editor.showTools ? 'auto' : '0%')+ '">' + topHtml + '</span>' : '',
