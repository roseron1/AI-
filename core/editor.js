...
( function() {
	...
	function Editor( instanceConfig, element, mode ) {
		...
		this.on( 'selectionChange', function updateTargetOfOtherEvents1( evt ) {
			updateCommandsContext( this, evt.data.path );
		} );
		this.on( 'activeFilterChange', function updateTargetOfOtherEvents2() {
	...
	function updateCommandsContext( editor, path, forceRefresh ) {
		...
		for ( name in commands ) {
			...
			if ( (forceRefresh || command.contextSensitive) && !command.frozen )
	...
	function loadLang( editor ) {
		CKEDITOR.lang.load( editor.config.language, editor.config.defaultLanguage, function( languageCode, lang ) {
			...
			editor.title = typeof configTitle == 'string' || configTitle === false ? configTitle : editor.lang.editor;
	...
	CKEDITOR.tools.extend( CKEDITOR.editor.prototype, {
		...
		setData: function( data, options, internal ) {
			...
			if ( !internal && fireSnapshot )
				this.fire( 'saveSnapshot', {name: 'setData', dataReady: false});

			if ( callback || !internal ) {
				this.once( 'dataReady', function( evt ) {
					if ( !internal && fireSnapshot )
						this.fire( 'saveSnapshot', {name: 'setData', dataReady: true});
		 ...
		 */
		insertHtml: function( html, mode, range, doNotFocus ) {
			this.fire( 'insertHtml', { dataValue: html, mode: mode, range: range, doNotFocus: doNotFocus } );
		},
