...
( function() {
	...
	function rangeRequiresFix( range ) {return false// todo ???????????????????? 啥意思啊, 加零宽字符
	...
	function getOnKeyDownListener( editor ) {
		...
		return function fakingSelection( evt ) {
			...
			if ( next && next.type == CKEDITOR.NODE_ELEMENT && next.getAttribute( 'data-hm-node' ) ){
				var $startContainer = $(range.startContainer.$);
				var $container = $startContainer.is('p,td') ? $startContainer : $startContainer.parents('p,td').first();
				var $next = $(next.$);
				var $nextContainer = $next.parents('p,td').first();
				if( !$container.is($nextContainer) ){
					switch(keystroke){
						case 8:
							var $nextNext = $next.next();
							$nextNext.length == 0 && $next.after('\u200B');
							return;
						break;
					}
	...
	CKEDITOR.on( 'instanceCreated', function( ev ) {
		...
		editor.on( 'contentDom', function() {
			...
			editable.attachListener( editable, 'keydown', function checkEditable( evt ) {
			...
			if ( CKEDITOR.env.webkit ) {
				...
				editable.attachListener( doc, 'keydown', function removeFillingCharSequenceNode1( evt ) {
		...
		editor.on( 'dataReady', function checkSelectionChange1() {
		...
		editor.on( 'key', function fakeSelection1( evt ) {
	...
	if ( CKEDITOR.env.webkit ) {
		CKEDITOR.on( 'instanceReady', function( evt ) {
			...
			// 检测零宽字符?
			editor.on( 'selectionChange', function check_u200b() {
	...
	CKEDITOR.dom.selection.prototype = {
		 ...
		 * @param {boolean} dontFireChange 不触发 selectionChange 事件: 分页后的选区恢复并不会影响与选区相关的业务逻辑.
		 * representing ranges to be added to the document.
		 */
		// YanJunwei - ckeditor 在使用实体书签设置选择范围时, 在特定情况下会添加零宽字符. 目前暂时跳过此函数(rangeRequiresFix), 造成的影响未知.
		selectRanges: function( ranges, dontFireChange ) {
			var editor = this.root.editor,
			...
			dontFireChange || this.root.fire( 'selectionchange' );
		},
		 ...
		 */
		selectBookmarks: function( bookmarks, dontFireChange ) {
			var ranges = [],
				node;
			...
			if ( bookmarks.isFake && !isTableSelection( ranges ) ) {
				...
				this.selectRanges( ranges, dontFireChange );
			}
